...
class MineralModesDatabaseQt6(QMainWindow):
    ...
    def setup_left_panel(self, parent):
        ...
        save_btn.setStyleSheet("QPushButton { background-color: #2e7d32; color: white; }")
            ...
            for tensor_idx, tensor_id in enumerate(valid_tensors):
                try:
                    ...
                    if tensor_type.startswith("High-frequency"):
                        ...
                        properties_text += "Static dielectric properties:\n"
                        properties_text += f"Average dielectric constant: {np.mean(eigenvalues):.4f}\n"
                        properties_text += f"Dielectric anisotropy: {(max(eigenvalues) - min(eigenvalues))/np.mean(eigenvalues):.4f}\n"
        ...
        try:
            ...
            for atom_type in sorted(valid_atoms):  # Sort for consistent display
                ...
                try:
                    ...
                    properties_text += f"Error processing {atom_type} tensor: {str(e)}\n"
                    # Show available components for debugging
                    properties_text += f"  Available components: {list(components.keys())}\n"
                    for comp, values in components.items():
                        properties_text += f"    {comp}: {values}\n"
                    properties_text += "\n"
                    current_row += 1
