...
class RamanClusterAnalysisWindow:
    """Window for Raman cluster analysis with hierarchical clustering."""
    ...
    def __init__(self, parent, raman_app):
        ...
        self.window.title("Raman Cluster Analysis")
        ...
        # Early initialization: Load CSV classifications if available
        print("DEBUG: Initializing RamanClusterAnalysisWindow on Windows")
        try:
            self._csv_hey_classes = self.load_hey_classifications_from_csv()
            if self._csv_hey_classes:
                print(f"DEBUG: Loaded {len(self._csv_hey_classes)} CSV classifications during initialization")
            else:
                print("DEBUG: No CSV classifications loaded during initialization")
        except Exception as e:
            print(f"DEBUG: Error loading CSV classifications during initialization: {str(e)}")
            self._csv_hey_classes = set()
        ...
        self.create_pca_components_tab()  # Add new PCA components tab
        self.create_analysis_tab()
        self.create_refinement_tab()
        
        # Create status bar
        self.status_bar = ttk.Label(self.window, text="Initializing...", relief=tk.SUNKEN, anchor=tk.W)
        ...
        # Update database status (this will try to find the database)
    ...
    def export_analysis_results(self):
        ...
        if self.cluster_data is None or 'labels' not in self.cluster_data or self.cluster_data['labels'] is None:
            messagebox.showinfo("Info", "No clustering results available to export.", parent=self.window)
            return
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Analysis results exported to {file_path}", parent=self.window)
    ...
    def create_refinement_tab(self):
        ...
        instructions_text = ("Instructions:\n"
                           "1. Choose visualization method: PCA (linear) or UMAP (non-linear, better cluster separation)\n"
                           "2. Click points in the scatter plot to select/deselect them\n"
                           "3. Selected points will be highlighted in red\n"
                           "4. Use 'Merge Selected Clusters' to combine clusters containing selected points\n"
                           "5. Use 'Split Selected Cluster' to divide a cluster into subclusters\n"
                           "6. 'Reset Selection' clears all selected points\n"
                           "7. 'Apply Changes' updates the main analysis with refinement results\n\n"
                           "Tip: UMAP often reveals better cluster structure for complex spectral data!")
        ...
        # Add visualization method selector for refinement
        viz_method_frame = ttk.Frame(control_frame)
        viz_method_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(viz_method_frame, text="Visualization Method:").pack(side=tk.LEFT, padx=5)
        self.refinement_viz_method = tk.StringVar(value='PCA')
        
        # Determine available methods for refinement
        refinement_viz_methods = ['PCA']
        if UMAP_AVAILABLE:
            refinement_viz_methods.append('UMAP')
        
        refinement_viz_combo = ttk.Combobox(viz_method_frame, textvariable=self.refinement_viz_method, 
                                          values=refinement_viz_methods, width=10)
        refinement_viz_combo.pack(side=tk.LEFT, padx=5)
        refinement_viz_combo.bind('<<ComboboxSelected>>', lambda e: self.update_refinement_visualization())
        
        # Add update visualization button
        ttk.Button(viz_method_frame, text="Update Visualization", 
                  command=self.update_refinement_visualization).pack(side=tk.LEFT, padx=10)
    ...
    def run_clustering(self):
        ...
        if self.cluster_data is None or self.cluster_data['features'] is None:
            messagebox.showinfo("Info", "No data available for clustering", parent=self.window)
            return
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("Error", "Please enter a valid number of clusters (minimum 2)", parent=self.window)
            ...
            messagebox.showerror("Error", f"Clustering failed: {str(e)}", parent=self.window)
    ...
    def update_visualizations(self):
        ...
        try:
            ...
            # Update PCA components plot if it exists
            if hasattr(self, 'pca_comp_canvas'):
        ...
        try:
            ...
            try:
                if method == 'PCA':
                    ...
                    if method == 'UMAP' and not UMAP_AVAILABLE:
                        ...
                        messagebox.showinfo("Info", 
                            "UMAP is not available. Install with 'pip install umap-learn'. Using PCA instead.", parent=self.window)
                    else:
                        print(f"DEBUG: Unsupported visualization method: {method}")
                        messagebox.showinfo("Info", f"Visualization method '{method}' not supported. Using PCA instead.", parent=self.window)
                ...
                messagebox.showerror("Memory Error", 
                    f"Insufficient memory for {method} with {n_samples} samples. Try PCA instead.", parent=self.window)
            ...
            import traceback
            traceback.print_exc()
            messagebox.showerror("Visualization Error", f"Error updating scatter plot: {str(e)}", parent=self.window)

    def update_pca_components_plot(self):
        """Update the PCA components loadings plot."""
        try:
            if self.cluster_data is None or self.cluster_data['features'] is None:
                self.pca_comp_ax.clear()
                self.pca_comp_ax.text(0.5, 0.5, "No data available.\nRun clustering first.", 
                                     ha='center', va='center', transform=self.pca_comp_ax.transAxes,
                                     fontsize=14)
                self.pca_comp_canvas.draw()
                return
            
            # Get the features and wavenumbers
            features = self.cluster_data['features_scaled']
            if features is None:
                features = StandardScaler().fit_transform(self.cluster_data['features'])
            
            wavenumbers = self.cluster_data['wavenumbers']
            
            # Perform PCA to get component loadings
            pca = PCA(n_components=min(10, features.shape[1]))  # Max 10 components
            pca.fit(features)
            ...
            n_components = min(self.pca_components_to_show.get(), len(pca.components_))
            ...
            for i in range(n_components):
                ...
                self.pca_comp_ax.plot(wavenumbers, loadings, 
                                     color=colors[i], linewidth=2, 
                                     label=f'PC{i+1} ({var_explained:.1%} var.)')
            
            # Customize the plot
            self.pca_comp_ax.set_xlabel('Wavenumber (cm⁻¹)', fontsize=12)
            self.pca_comp_ax.set_ylabel('Loading Value', fontsize=12)
            self.pca_comp_ax.set_title('PCA Component Loadings vs Wavenumbers', fontsize=14)
            self.pca_comp_ax.grid(True, alpha=0.3)
            self.pca_comp_ax.legend()
            
            # Add horizontal line at zero
            self.pca_comp_ax.axhline(y=0, color='black', linestyle='-', alpha=0.3)
            
            # Set x-axis to show wavenumbers in descending order (common for Raman)
            self.pca_comp_ax.invert_xaxis()
            
            # Tight layout and draw
            self.pca_comp_fig.tight_layout()
            self.pca_comp_canvas.draw()
            
        except Exception as e:
            print(f"DEBUG: PCA components plot update error: {str(e)}")
            self.pca_comp_ax.clear()
            self.pca_comp_ax.text(0.5, 0.5, f"Error updating plot:\n{str(e)}", 
                                 ha='center', va='center', transform=self.pca_comp_ax.transAxes,
    ...
    def export_visualization(self):
        ...
        try:
            if not hasattr(self, 'viz_fig') or self.viz_fig is None:
                messagebox.showinfo("Info", "No visualization available to export.", parent=self.window)
                return
            ...
            messagebox.showerror("Error", f"Failed to export visualization: {str(e)}", parent=self.window)
            print(f"DEBUG: Export visualization error: {str(e)}")

    def start_refinement(self):
        """Start the refinement mode."""
        if self.cluster_data is None:
            messagebox.showerror("Error", "Please run clustering first.", parent=self.window)
            return
        ...
        messagebox.showinfo("Refinement Mode", 
                          "Refinement mode activated. Select points in the scatter plot to merge clusters.", parent=self.window)
    ...
    def undo_last_action(self):
        ...
        if not self.undo_stack:
            messagebox.showinfo("Info", "No actions to undo.", parent=self.window)
            return
        ...
        messagebox.showinfo("Success", "Last action undone.", parent=self.window)
    ...
    def preview_split(self):
        ...
        if not hasattr(self, 'selected_points') or not self.selected_points:
            messagebox.showinfo("Info", "No points selected for splitting.\n\nDouble-click points in the visualization to select them.", parent=self.window)
            return
        ...
        if len(selected_clusters) != 1:
            messagebox.showinfo("Info", f"Please select points from only one cluster to split.\n\n" +
                              f"Currently selected points are from {len(selected_clusters)} different clusters.", parent=self.window)
            return
        ...
        messagebox.showinfo("Preview", 
                          "Split preview applied. Use 'Apply Preview' to keep changes or 'Cancel Preview' to revert.", parent=self.window)
    ...
    def apply_preview(self):
        ...
        if not self.undo_stack:
            messagebox.showinfo("Info", "No preview to apply.", parent=self.window)
            return
            
        # Keep the current state
        self.undo_stack.pop()  # Remove the saved state since we're keeping the changes
        messagebox.showinfo("Success", "Preview changes applied.", parent=self.window)

    def cancel_preview(self):
        """Cancel the previewed changes."""
        if not self.undo_stack:
            messagebox.showinfo("Info", "No preview to cancel.", parent=self.window)
            return
        ...
        messagebox.showinfo("Success", "Preview cancelled.", parent=self.window)
    ...
    def split_selected_cluster(self, preview=False):
        ...
        if self.cluster_data is None or 'labels' not in self.cluster_data or self.cluster_data['labels'] is None:
            messagebox.showinfo("Info", "No clustering data available. Please run clustering first.", parent=self.window)
            return
        ...
        if len(cluster_points) < n_subclusters:
            messagebox.showinfo("Info", f"Cannot split cluster {cluster_to_split} into {n_subclusters} subclusters.\n\n" +
                              f"The cluster only has {len(cluster_points)} points.", parent=self.window)
            return
        ...
        if not preview:
            ...
            if not messagebox.askyesno("Confirm Split", split_msg, parent=self.window):
                return
        ...
        try:
            ...
            if not preview:
                ...
                messagebox.showinfo("Success", 
                                  f"Successfully split cluster {cluster_to_split} into {n_subclusters} subclusters using {split_method}.", parent=self.window)
            
        except Exception as e:
            print(f"DEBUG: Error during cluster split: {str(e)}")
            messagebox.showerror("Error", f"Failed to split cluster: {str(e)}", parent=self.window)
    ...
    def merge_selected_clusters(self):
        ...
        if len(selected_clusters) < 2:
            messagebox.showinfo("Info", "Please select points from at least 2 different clusters to merge.\n\n" +
                              f"Currently selected points are from {len(selected_clusters)} cluster(s).", parent=self.window)
            return
        ...
        if not messagebox.askyesno("Confirm Merge", merge_msg, parent=self.window):
        ...
        messagebox.showinfo("Success", f"Successfully merged clusters {cluster_list} into cluster {new_label}.", parent=self.window)
    ...
    def apply_refinement(self):
        ...
        messagebox.showinfo("Success", "Refinement changes applied successfully.", parent=self.window)
        ...
        try:
            ...
            if hasattr(self.raman_app, 'current_spectrum') and self.raman_app.current_spectrum is not None:
                ...
                if intensities.shape[0] == 1:
                    ...
                    messagebox.showinfo("Info", 
                        "Single spectrum imported. For better clustering results, consider importing multiple spectra.", parent=self.window)
                else:
                ...
                messagebox.showinfo("Info", "No spectrum data available in main application.", parent=self.window)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to import data: {str(e)}", parent=self.window)
    ...
    def import_from_database(self):
        ...
        try:
            ...
            if not db_items:
                messagebox.showinfo("Info", "No items in database.", parent=self.window)
                return
            ...
            def import_selected():
                ...
                if not selected_indices:
                    messagebox.showinfo("Info", "Please select items to import.", parent=self.window)
                    return
    ...
    def select_import_folder(self):
        ...
        folder = filedialog.askdirectory(parent=self.window, title="Select Folder with Spectrum Files")
        if folder:
    ...
    def start_batch_import(self):
        ...
        if not hasattr(self, 'selected_folder') or not self.selected_folder:
            messagebox.showinfo("Info", "Please select a folder first", parent=self.window)
            return
        ...
        try:
            ...
            if not files:
                messagebox.showinfo("Info", f"No valid files found in {self.selected_folder}", parent=self.window)
                return
            ...
            if not all_wavenumbers:
                messagebox.showinfo("Info", "No valid data found in any files", parent=self.window)
                return
            ...
            messagebox.showerror("Error", f"Import failed: {str(e)}", parent=self.window)
    ...
    def append_data(self):
        ...
        if self.cluster_data is None:
            messagebox.showinfo("Info", "Please import initial data first.", parent=self.window)
            return
        ...
        def append_from_file():
            ...
            if file_path:
                try:
                    ...
                    messagebox.showerror("Error", f"Failed to append data: {str(e)}", parent=self.window)
        ...
        def append_from_folder():
            ...
            if folder:
                try:
                    ...
                    if not files:
                        messagebox.showinfo("Info", "No valid files found in selected folder.", parent=self.window)
                        return
                    ...
                    if all_wavenumbers:
                        ...
                        if failed_files:
                            ...
                            messagebox.showwarning("Import Warnings", error_msg, parent=self.window)
    ...
    def append_spectra(self, wavenumbers, intensities):
        ...
        try:
            ...
            if self.cluster_data['wavenumbers'] is None or self.cluster_data['intensities'] is None:
                ...
                messagebox.showinfo("Success", f"Successfully imported {intensities.shape[0]} spectra.", parent=self.window)
                return
            ...
            messagebox.showinfo("Success", 
                              f"Successfully appended {intensities.shape[0]} spectra.", parent=self.window)
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to append spectra: {str(e)}", parent=self.window)
            raise
    ...
    def locate_database_file(self):
        """Allow user to manually locate the database.pkl file with Windows compatibility."""
        try:
            import pickle
            import os
            import platform
            ...
            print("DEBUG: Starting locate_database_file()")
            
            # Determine starting directory based on platform
            if platform.system() == "Windows":
                # On Windows, start in Documents folder
                home_dir = os.path.expanduser("~")
                initial_dir = os.path.join(home_dir, "Documents")
                if not os.path.exists(initial_dir):
                    initial_dir = home_dir
            else:
                # On other platforms, start in home directory
                initial_dir = os.path.expanduser("~")
            
            print(f"DEBUG: Starting file dialog in: {initial_dir}")
            
            # Prompt user to select database file
            db_path = filedialog.askopenfilename(
                parent=self.window,
                title="Select Raman Database File",
            ...
            print(f"DEBUG: User selected file: {db_path}")
            
            # Verify file exists
            if not os.path.exists(db_path):
                print(f"DEBUG: Selected file does not exist: {db_path}")
                messagebox.showerror("File Not Found", f"The selected file does not exist:\n{db_path}", parent=self.window)
                self.status_bar.config(text="Selected database file not found")
                return False
            
            # Try to load the database with multiple approaches
            database = None
            loading_errors = []
            
            # Method 1: Standard pickle load
            try:
                print("DEBUG: Trying standard pickle load...")
                with open(db_path, 'rb') as f:
                    database = pickle.load(f)
                print(f"DEBUG: Standard pickle load successful, type: {type(database)}")
            except Exception as e:
                loading_errors.append(f"Standard pickle: {str(e)}")
                print(f"DEBUG: Standard pickle load failed: {str(e)}")
            
            # Method 2: Try with different protocols if standard failed
            if database is None:
                for protocol in [0, 1, 2, 3, 4, 5]:
                    try:
                        print(f"DEBUG: Trying pickle load with protocol {protocol}...")
                        with open(db_path, 'rb') as f:
                            database = pickle.load(f)
                        print(f"DEBUG: Pickle load with protocol {protocol} successful")
                        break
                    except Exception as e:
                        loading_errors.append(f"Protocol {protocol}: {str(e)}")
                        print(f"DEBUG: Pickle load with protocol {protocol} failed: {str(e)}")
                        continue
            
            # Check if we successfully loaded something
            if database is None:
                error_msg = "Failed to load database file. Errors encountered:\n\n"
                for i, error in enumerate(loading_errors[:5], 1):  # Show first 5 errors
                    error_msg += f"{i}. {error}\n"
                if len(loading_errors) > 5:
                    error_msg += f"... and {len(loading_errors) - 5} more errors"
                
                print(f"DEBUG: All loading attempts failed")
                messagebox.showerror("Database Load Error", error_msg, parent=self.window)
                self.status_bar.config(text="Failed to load selected database file")
                return False
            
            # Validate that it's a proper database
            if not isinstance(database, dict):
                print(f"DEBUG: Loaded object is not a dictionary, type: {type(database)}")
                messagebox.showerror("Invalid Database", 
                    f"The selected file does not contain a valid Raman database.\n\nExpected: Dictionary\nFound: {type(database).__name__}", 
                    parent=self.window)
                self.status_bar.config(text="Invalid database file format")
                return False
            
            if not database:
                print("DEBUG: Database dictionary is empty")
                messagebox.showerror("Empty Database", 
                    "The selected database file is empty.", 
                    parent=self.window)
                self.status_bar.config(text="Empty database file")
                return False
            
            print(f"DEBUG: Database validation successful, {len(database)} entries found")
            ...
            if hasattr(self.raman_app, 'raman'):
                ...
                print("DEBUG: Set database reference in main app")
            
            # Extract Hey classifications for immediate use
            hey_classes = set()
            ...
            print(f"DEBUG: Extracted {len(hey_classes)} Hey classifications from database")
            
            # Success message
            success_msg = f"Successfully loaded database with {len(database)} entries from:\n{os.path.basename(db_path)}"
            if hey_classes:
                success_msg += f"\n\nFound {len(hey_classes)} different Hey classifications"
            
            messagebox.showinfo("Database Loaded", success_msg, parent=self.window)
            
            # Update status bar
            self.status_bar.config(text=f"Using custom database: {os.path.basename(db_path)} ({len(database)} entries)")
            
            print("DEBUG: locate_database_file completed successfully")
            return True  # Success
            
        except Exception as e:
            print(f"DEBUG: Unexpected error in locate_database_file: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def get_database(self):
        """Return the database from the main app, handling nested raman attribute with Windows compatibility."""
        print("DEBUG: Starting get_database() method")
        ...
        if hasattr(self.raman_app, 'raman') and hasattr(self.raman_app.raman, 'database'):
            ...
            if db:
                print(f"DEBUG: Found database in main app with {len(db)} entries")
                self.status_bar.config(text=f"Using database from main app: {len(db)} entries")
                return db
        
        # If we have a custom path that was manually set, try that first
        if hasattr(self, 'custom_db_path') and self.custom_db_path:
            if os.path.exists(self.custom_db_path):
                try:
                    print(f"DEBUG: Trying to load custom database from: {self.custom_db_path}")
                    with open(self.custom_db_path, 'rb') as f:
                        database = pickle.load(f)
                    print(f"DEBUG: Successfully loaded custom database with {len(database)} entries")
                    self.status_bar.config(text=f"Using custom database: {self.custom_db_path} ({len(database)} entries)")
                    return database
                except Exception as e:
                    print(f"DEBUG: Failed to load custom database: {str(e)}")
        
        # Try multiple possible database locations (Windows-compatible)
        try:
            import pickle
            import os
            import platform
            ...
            # Define possible database paths (Windows-compatible)
            possible_paths = []
            
            # 1. Home directory (use proper Windows paths)
            home_dir = os.path.expanduser("~")
            possible_paths.append(os.path.join(home_dir, "raman_database.pkl"))
            
            # 2. Script directory
            possible_paths.append(os.path.join(script_dir, "raman_database.pkl"))
            
            # 3. Parent directory of script
            parent_dir = os.path.dirname(script_dir)
            possible_paths.append(os.path.join(parent_dir, "raman_database.pkl"))
            
            # 4. Common Windows application data directories
            if platform.system() == "Windows":
                import os
                # Try Documents folder
                documents_dir = os.path.join(home_dir, "Documents")
                possible_paths.append(os.path.join(documents_dir, "raman_database.pkl"))
                
                # Try Desktop
                desktop_dir = os.path.join(home_dir, "Desktop")
                possible_paths.append(os.path.join(desktop_dir, "raman_database.pkl"))
                
                # Try AppData folders
                appdata_local = os.environ.get('LOCALAPPDATA')
                if appdata_local:
                    possible_paths.append(os.path.join(appdata_local, "raman_database.pkl"))
                    
                appdata_roaming = os.environ.get('APPDATA')
                if appdata_roaming:
                    possible_paths.append(os.path.join(appdata_roaming, "raman_database.pkl"))
            
            # 5. Current working directory
            possible_paths.append("raman_database.pkl")
            
            print(f"DEBUG: Checking {len(possible_paths)} possible database locations:")
            for i, path in enumerate(possible_paths):
                print(f"DEBUG: {i+1}. {path}")
            
            # Try each path
            for i, db_path in enumerate(possible_paths):
                try:
                    if os.path.exists(db_path):
                        print(f"DEBUG: Found database file at: {db_path}")
                        ...
                        if isinstance(database, dict) and database:
                            print(f"DEBUG: Successfully loaded database with {len(database)} entries from {db_path}")
                            # Store this path for future use
                            self.custom_db_path = db_path
                            self.status_bar.config(text=f"Using database: {os.path.basename(db_path)} ({len(database)} entries)")
                            return database
                        else:
                            print(f"DEBUG: File exists but doesn't contain valid database: {db_path}")
                    else:
                        print(f"DEBUG: File not found: {db_path}")
                except Exception as e:
                    print(f"DEBUG: Error loading database from {db_path}: {str(e)}")
                    continue
            
            # If no database found, show a helpful message
            print("DEBUG: No database found in any location")
            self.status_bar.config(text="No database found - use 'Locate Database File' to browse for it")
            
        except Exception as e:
            print(f"DEBUG: Exception in get_database: {str(e)}")
            self.status_bar.config(text=f"Error searching for database: {str(e)}")
            
        return None

    def get_hey_classifications(self):
        """Return a list of Hey classifications primarily from the database entries with Windows compatibility."""
        print("DEBUG: Starting get_hey_classifications()")
        ...
        # Try to load CSV classifications first (this provides the most comprehensive list)
        csv_hey_classes = set()
        if not hasattr(self, '_csv_hey_classes') or not self._csv_hey_classes:
            print("DEBUG: Loading Hey classifications from CSV file...")
            csv_hey_classes = self.load_hey_classifications_from_csv()
            if csv_hey_classes:
                print(f"DEBUG: Loaded {len(csv_hey_classes)} classifications from CSV")
            else:
                print("DEBUG: No classifications found in CSV file")
        else:
            csv_hey_classes = self._csv_hey_classes
            print(f"DEBUG: Using cached CSV classifications: {len(csv_hey_classes)} entries")
        
        db = self.get_database()
        db_hey_classes = set()
        
        if db is not None and db:
            print(f"DEBUG: Extracting Hey classifications from database with {len(db)} entries")
            ...
            # Debug output
            print(f"DEBUG: Hey Classifications found in database: {len(db_classes)}")
            for cls in sorted(list(db_classes)[:10]):  # Show first 10
                print(f"DEBUG:   - '{cls}'")
            if len(db_classes) > 10:
                print(f"DEBUG:   ... and {len(db_classes) - 10} more")
            
            print(f"DEBUG: Standardized Hey Classifications from database: {len(db_hey_classes)}")
        else:
            print("DEBUG: No database available for Hey classifications")
        
        # Combine all sources of classifications without duplicates
        all_hey_classes = set()
        
        # Add database classifications first (they're the most up-to-date and specific to the actual data)
        all_hey_classes.update(db_hey_classes)
        print(f"DEBUG: Added {len(db_hey_classes)} database classifications")
        
        # Add CSV classifications (provides comprehensive coverage)
        all_hey_classes.update(csv_hey_classes)
        print(f"DEBUG: Total after adding CSV classifications: {len(all_hey_classes)}")
        
        # Add defaults for any that might be missing
        initial_count = len(all_hey_classes)
        for cls in default_classifications:
            if cls not in all_hey_classes:
                all_hey_classes.add(cls)
        print(f"DEBUG: Added {len(all_hey_classes) - initial_count} default classifications")
        
        # Sort and return as a list
        result = sorted(all_hey_classes)
        print(f"DEBUG: Final Hey classifications list has {len(result)} entries")
        print("DEBUG: Sample final classifications:", result[:10] if len(result) >= 10 else result)
    ...
    def import_from_database_with_filters(self, hey_class, must_have, only, not_these):
        ...
        if db is None and hasattr(self, 'custom_db_path') and os.path.exists(self.custom_db_path):
            try:
                ...
                messagebox.showerror("Error", f"Failed to load database from custom path: {str(e)}", parent=self.window)
        
        if db is None:
            messagebox.showerror("Error", "No database found. Please locate a database file using 'Locate Database File' button.", parent=self.window)
            return
        ...
        if not filtered:
            ...
            messagebox.showinfo("No Matches Found", no_match_msg, parent=self.window)
            return
        ...
        if not all_spectra:
            progress_window.destroy()
            messagebox.showinfo("Info", "No valid spectra could be processed.", parent=self.window)
            return
        ...
        if failed_spectra:
            ...
            messagebox.showwarning("Import Warnings", warning_msg, parent=self.window)
        
        # Update UI
        self.update_ui_after_import(len(all_spectra))

    def update_database_status(self):
        """Update the database status in the status bar with enhanced Windows information."""
        print("DEBUG: Updating database status")
        
        # Try to get the database
        db = self.get_database()
        
        # Check CSV classifications
        csv_count = len(self._csv_hey_classes) if hasattr(self, '_csv_hey_classes') and self._csv_hey_classes else 0
        
        if db:
            db_count = len(db)
            print(f"DEBUG: Database found with {db_count} entries")
            
            # Extract Hey classifications from database for status
            db_hey_classes = set()
            for entry in db.values():
                if 'metadata' in entry and isinstance(entry['metadata'], dict):
                    hey_class = entry['metadata'].get('HEY CLASSIFICATION', '')
                    if hey_class:
                        db_hey_classes.add(hey_class)
                elif 'hey_classification' in entry and entry['hey_classification']:
                    db_hey_classes.add(entry['hey_classification'])
            
            db_hey_count = len(db_hey_classes)
            
            if csv_count > 0:
                status_text = f"Database: {db_count} entries, {db_hey_count} Hey classes | CSV: {csv_count} Hey classes"
            else:
                status_text = f"Database loaded: {db_count} entries, {db_hey_count} Hey classifications"
                
            self.status_bar.config(text=status_text)
            print(f"DEBUG: Status updated - DB: {db_count} entries, {db_hey_count} DB Hey classes, {csv_count} CSV Hey classes")
        else:
            print("DEBUG: No database found")
            if csv_count > 0:
                status_text = f"No database loaded | CSV: {csv_count} Hey classifications available"
            else:
                status_text = "No database loaded - use 'Locate Database File' or check console for path info"
            
            self.status_bar.config(text=status_text)
            print(f"DEBUG: Status updated - No DB, {csv_count} CSV Hey classes")

    def load_hey_classifications_from_csv(self):
        """Load Hey Classifications directly from the RRUFF export CSV file with Windows compatibility."""
        try:
            import os
            import csv
            import platform
            
            print("DEBUG: Starting load_hey_classifications_from_csv()")
            
            # Get the current script directory
            script_dir = os.path.dirname(os.path.abspath(__file__))
            home_dir = os.path.expanduser("~")
            
            # Define possible file paths (Windows-compatible)
            possible_paths = [
                # Current directory
                "RRUFF_Export_with_Hey_Classification.csv",
                # Script directory  
                os.path.join(script_dir, "RRUFF_Export_with_Hey_Classification.csv"),
                # Parent directory of script
                os.path.join(os.path.dirname(script_dir), "RRUFF_Export_with_Hey_Classification.csv"),
                # Home directory
                os.path.join(home_dir, "RRUFF_Export_with_Hey_Classification.csv"),
            ]
            ...
            if platform.system() == "Windows":
                # Documents folder
                documents_dir = os.path.join(home_dir, "Documents")
                possible_paths.append(os.path.join(documents_dir, "RRUFF_Export_with_Hey_Classification.csv"))
                
                # Desktop
                desktop_dir = os.path.join(home_dir, "Desktop")
                possible_paths.append(os.path.join(desktop_dir, "RRUFF_Export_with_Hey_Classification.csv"))
                
                # Downloads folder
                downloads_dir = os.path.join(home_dir, "Downloads")
                possible_paths.append(os.path.join(downloads_dir, "RRUFF_Export_with_Hey_Classification.csv"))
            
            print(f"DEBUG: Checking {len(possible_paths)} possible CSV file locations:")
            for i, path in enumerate(possible_paths):
                print(f"DEBUG: {i+1}. {path}")
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    csv_path = path
                    print(f"DEBUG: Found CSV file at: {csv_path}")
                    break
            
            if not csv_path:
                print("DEBUG: Could not find RRUFF_Export_with_Hey_Classification.csv in any location")
                return set()
            
            # Read classifications from the CSV with multiple encoding attempts
            hey_classes = set()
            
            # Try different encodings that are common on Windows
                ...
                try:
                    ...
                    with open(csv_path, 'r', encoding=encoding, newline='') as f:
                        ...
                        try:
                            ...
                            # Fallback to default CSV reader
                            reader = csv.reader(f)
                        
                        # Skip header
                        try:
                            header = next(reader)
                            print(f"DEBUG: CSV header has {len(header)} columns")
                            print(f"DEBUG: Header sample: {header[:5] if len(header) >= 5 else header}")
                        except StopIteration:
                            print("DEBUG: CSV file appears to be empty")
                            continue
                        ...
                        for row in reader:
                            ...
                            if len(row) >= 21:  # Ensure we have enough columns
                                # Get the classification name from the 21st column (0-indexed as 20)
                                hey_class = str(row[20]).strip()
                                if hey_class and hey_class != "Hey Classification Name" and hey_class.lower() != "unknown":
                                    hey_classes.add(hey_class)
                            
                            # Show progress for large files
                            if row_count % 1000 == 0:
                                print(f"DEBUG: Processed {row_count} rows, found {len(hey_classes)} unique classifications")
                    
                    print(f"DEBUG: Successfully read CSV with encoding {encoding}")
                    print(f"DEBUG: Processed {row_count} total rows")
                    ...
                    print(f"DEBUG: Error reading CSV with encoding {encoding}: {str(e)}")
                    continue
            
            if hey_classes:
                print(f"DEBUG: Loaded {len(hey_classes)} Hey Classifications from CSV file")
                print("DEBUG: Sample classifications:", sorted(list(hey_classes))[:10])
                # Store for later use
                self._csv_hey_classes = hey_classes
                return hey_classes
            else:
                print("DEBUG: No Hey Classifications found in CSV file")
                return set()
            
        except Exception as e:
            print(f"DEBUG: Error loading Hey Classifications from CSV: {str(e)}")
            import traceback
            traceback.print_exc()
            return set()
    ...
    def update_refinement_visualization(self):
        ...
        try:
            ...
            # Use the independent refinement visualization method
            method = self.refinement_viz_method.get() if hasattr(self, 'refinement_viz_method') else 'PCA'
            features_to_use = self.cluster_data['features_scaled']
    ...
    def create_pca_components_tab(self):
        """Create the PCA components tab for showing component loadings."""
        pca_tab = ttk.Frame(self.notebook)
        self.notebook.add(pca_tab, text="PCA Components")
        
        # Control frame
        control_frame = ttk.LabelFrame(pca_tab, text="Component Controls", padding=10)
        ...
        ttk.Label(comp_frame, text="Components to Show:").pack(side=tk.LEFT, padx=5)
        self.pca_components_to_show = tk.IntVar(value=2)
        comp_spin = ttk.Spinbox(comp_frame, textvariable=self.pca_components_to_show, 
                               from_=1, to=10, width=5)
        comp_spin.pack(side=tk.LEFT, padx=5)
        
        # Update button
        update_btn = ttk.Button(comp_frame, text="Update Plot", 
                               command=self.update_pca_components_plot)
        update_btn.pack(side=tk.LEFT, padx=10)
        
        # Plot frame
        plot_frame = ttk.LabelFrame(pca_tab, text="PCA Component Loadings")
        plot_frame.pack(fill="both", expand=True, padx=5, pady=5)
        
        # Create figure for PCA components
        self.pca_comp_fig = Figure(figsize=(10, 6))
        self.pca_comp_ax = self.pca_comp_fig.add_subplot(111)
        
        # Create canvas
        self.pca_comp_canvas = FigureCanvasTkAgg(self.pca_comp_fig, master=plot_frame)
        self.pca_comp_canvas.get_tk_widget().pack(fill="both", expand=True)
        
        # Add toolbar
        toolbar_frame = ttk.Frame(plot_frame)
        toolbar_frame.pack(fill=tk.X)
        pca_comp_toolbar = NavigationToolbar2Tk(self.pca_comp_canvas, toolbar_frame)
        pca_comp_toolbar.update()
        
        # Add help text
        help_frame = ttk.Frame(pca_tab)
        help_frame.pack(fill="x", padx=5, pady=5)
        
        help_text = ("This plot shows the PCA component loadings vs wavenumbers.\n"
                    "Positive values indicate wavenumbers that contribute positively to the component.\n"
                    "Negative values indicate wavenumbers that contribute negatively.\n"
                    "Large absolute values indicate wavenumbers that are most important for that component.")
        
        help_label = ttk.Label(help_frame, text=help_text, wraplength=800, 
